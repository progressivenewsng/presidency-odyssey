...
async function main() {
  ...
  const categoryMap = {
    Politics: categories.find((c: any) => c.slug === 'politics')!.id,
    Economy: categories.find((c: any) => c.slug === 'economy')!.id,
    Sports: categories.find((c: any) => c.slug === 'sports')!.id,
    Security: categories.find((c: any) => c.slug === 'security')!.id,
  };
  ...
  for (const postData of postsData) {
    const categoryId = categoryMap[postData.category as keyof typeof categoryMap];
  ...
  console.log('Created tags:', tags.map((t: any) => t.name));
